...
## 2026-04-18 WIKA 权限打通收口 Delta

### 当前仓内是否还有“下一批必须验证”的新对象
- 没有。
- 当前剩余问题已经不是“再测一个 API 可能就通”，而是：
  - missing official dimensions
  - 写侧前置条件不足
  - 广告 / 页面数据需要外部输入
...
- 新的官方文档与稳定参数契约
- 新的测试对象 / test scope
- 新的 rollback / cleanup / readback 证明
- 新的真实广告导出与页面盘点输入
...
### 本轮不再作为“下一批必须验证”的方向
- 广告 / 直通车数据：
  - 当前不继续做无文档、无稳定契约的官方候选试探
  - 先切换到 `IMPORT_REQUIRED` 路径
- 页面与内容优化：
  - 当前不去猜页面行为 API
  - 先复用既有产品经营、comparison、diagnostic 与 cockpit 信号做保守建议层

### 只有在什么条件下才值得把广告重开为官方候选
- 拿到稳定官方文档 URL
- 拿到稳定参数契约
- 拿到能直接对应 spend / impressions / clicks / inquiries / keyword 的方法说明
- 或拿到新的真实控制台 / 导出证据，足以证明存在当前可复用的官方入口
