...
| 外部回复草稿工作流 | 7 | 已形成只生成草稿的外部工作流层；基于真实 products 读侧、最小产品诊断和 alerts 结构生成 reply draft，不触发平台内发送 | `/integrations/alibaba/wika/tools/reply-draft` | 否 |
...
| 外部结构化报价单 / 订单草稿文档 | 7 | 已实现外部订单草稿 helper、样例与工具路由，但明确不等于平台内订单已创建 | `/integrations/alibaba/wika/tools/order-draft` | 否 |
...
4. 当前已经形成“外部回复草稿 + 外部订单草稿”这一层可直接使用的中间层，但它们都不等于平台内回复或平台内创单。
