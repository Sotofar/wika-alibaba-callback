...
6. 任务 4 / 5 当前已经形成“外部草稿工作流层”，后续若继续，应优先增强输入模板和 blocker 分层，而不是回到平台内写动作
...
- `/integrations/alibaba/wika/tools/reply-draft`
  - 当前已经形成外部回复草稿工具
  - 后续若继续任务 4，应优先增强输入模板、语言模板和 blocker 说明，而不是回到平台内回复发送
- `/integrations/alibaba/wika/tools/order-draft`
  - 当前已经形成外部订单草稿工具
  - 后续若继续任务 5，应优先增强外部订单包字段完整度和人工补齐说明，而不是回到平台内创单验证
...
当前最优先的下一批验证对象仍然是“真实 provider 外发验证”与“写侧可回滚证据”；而任务 4 / 5 当前已经有可直接使用的外部草稿工作流层，不应再把它们误写成平台内自动执行闭环。
