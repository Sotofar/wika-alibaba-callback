...
2. 在阶段 20 已确认 Railway production 基础路由连续超时/不可达之前，不继续推进任何新的 production access 验证
3. production 恢复后，先补任务 6：在真实 provider 已预接线的前提下，补一次低风险真实外发验证
4. 再补任务 3：只继续验证“可隔离、可清理、可回滚”的剩余写侧证据
5. 再补任务 4：只继续验证官方明确存在的 customers / inquiries / messages 读侧入口
6. 任务 5 当前已完成正式入口边界摸底；在出现新的官方低风险候选前，不再继续深挖 `order.create`
7. 任务 4 / 5 当前已经形成“模板化的外部草稿工作流 SOP 层”，后续若继续，应优先增强输入模板版本、人工补单 SOP、handoff checklist 和 blocker taxonomy，而不是回到平台内写动作
8. 阶段 17 已再次完成经营数据候选接口只读验证；阶段 18 已进一步把 `mydata / overview / self.product` 收口成权限清障包，当前不建议重开这条循环
9. 阶段 18 已进一步确认 `order.detail / fund / logistics` 的 public chaining 契约未闭合；在拿到可复用未遮罩订单 identifier 之前，不建议重开这条链的 runtime 验证
10. 阶段 19 已完整归类 `ICBU－商品 (cid=20966)` 左侧栏 47 页官方文档；新增“文档已确认存在，但尚未生产验证”的方法，只允许进入 doc-found 候选池，不能误报为主线已重开
11. 阶段 20 已证明当前最先要恢复的是 production 基础可用性，而不是继续追加 replay 轮次
...
- 阶段 20 只是在收口 WIKA access 多轮稳定化复跑、未决队列导出与 XD 标准权限预检，不代表任何新接口已经打通；当前如需继续，只应先恢复 Railway production 基础可用性。

## 阶段 21 增量说明
- 当前最先要做的不是新增 API 候选验证，而是按既有清单重开 WIKA replay
- 在 WIKA replay 回到接口级验证层前，不推进 XD 8 项逐项确认
